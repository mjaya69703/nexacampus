// Daftar permission — pilot kit CRUD shared (Inertia React).
import { Head, router } from '@inertiajs/react';
import { Braces, Globe, KeyRound, Plus, ShieldCheck, Trash2 } from 'lucide-react';
...
type Row = {
    id: number; no: number; name: string; guard: string; roleCount: number;
...
type Props = {
    ...
    stats: { total: number; web: number; api: number; attached: number; trashed: number };
    data: {
        rows: Row[];
...
const MODAL_COPY: Record<Exclude<Pending, null>['kind'], { title: (n: string) => string; message: (n: string, used: boolean) => string; confirm: string; danger: boolean }> = {
    delete: {
        ...
        message: (n, used) => `"${n}" dipindah ke sampah dan bisa dipulihkan.${used ? ' Permission ini masih dipakai role — penghapusan akan ditolak.' : ''}`,
        confirm: 'Ya, hapus',
        danger: true,
    },
    restore: {
        title: () => 'Pulihkan permission?',
        message: (n) => `"${n}" kembali aktif beserta relasi role-nya.`,
        confirm: 'Ya, pulihkan',
        danger: false,
    },
    force: {
        title: () => 'Hapus permanen?',
        message: (n, used) => `"${n}" dihapus selamanya dan tidak bisa dipulihkan.${used ? ' Permission ini masih dipakai role — penghapusan akan ditolak.' : ''}`,
        confirm: 'Ya, hapus permanen',
        danger: true,
    },
    'bulk-delete': {
        title: () => 'Hapus permission terpilih?',
        message: (n) => `${n} permission dipindah ke sampah. Yang masih dipakai role akan dilewati.`,
        confirm: 'Ya, hapus',
        danger: true,
    },
    'bulk-restore': {
        title: () => 'Pulihkan permission terpilih?',
        message: (n) => `${n} permission kembali aktif.`,
        confirm: 'Ya, pulihkan',
        danger: false,
    },
    'bulk-force': {
        title: () => 'Hapus permanen permission terpilih?',
        message: (n) => `${n} permission dihapus selamanya. Yang masih dipakai role akan dilewati.`,
        confirm: 'Ya, hapus permanen',
        danger: true,
    },
};

export default function PermissionIndex({ shell, can, stats, data, filters, urls }: Props) {
    ...
    const copyUsed = pending && 'row' in pending ? pending.row.roleCount > 0 : false;

    return (
        <AdminShell shell={shell}>
            <Head title={`Daftar Permission · ${shell.appName}`} />
            <div className="db-root">
                <div className="db-stack">
                    <CrudHero
                        ...
                        actions={can.create ? (
                            <a className="db-btn light" href={urls.create}><Plus size={15} /> Tambah Permission</a>
                        ) : undefined}
                    />

                    <section className="db-stats" aria-label="Statistik permission">
                        <div className="db-stat">
                            <span className="db-stat-icon"><ShieldCheck size={20} /></span>
                            <div><span className="db-stat-num">{stats.total}</span><span className="db-stat-label">Total permission</span></div>
                        </div>
                        <div className="db-stat">
                        ...
                        <div className="db-stat">
                            <span className="db-stat-icon gold"><Braces size={20} /></span>
                            <div><span className="db-stat-num">{stats.api}</span><span className="db-stat-label">Guard API</span></div>
                        </div>
                        <div className="db-stat">
                    ...
                    <section className="db-card">
                        <div className="db-card-head">
                            <h2 className="db-card-title">Tabel Permission</h2>
                        ...
                        <div className="db-card-body">
                            <CrudTable<Row>
                                columns={[
                                    ...
                                    { key: 'name', label: 'Nama hak akses', sortable: true, render: (row) => <span className="crud-name">{row.name}</span> },
                                    { key: 'guard_name', label: 'Guard', sortable: true, render: (row) => <span className={`db-badge ${row.guard === 'api' ? 'amber' : ''}`.trim()}>{row.guard}</span> },
                                    {
                                        key: 'role_count', label: 'Dipakai role', sortable: true, align: 'right',
                                        render: (row) => <span className={`db-badge ${row.roleCount > 0 ? 'green' : 'gray'}`}>{row.roleCount} role</span>,
                                    },
                                ...
                                searchPlaceholder="Cari nama permission (contoh: user.create)…"
                                filterBar={(
                                    <select
                                        className="db-input"
                                        value={filters.guard}
                                ...
                                emptyText={isTrash ? 'Sampah kosong. Permission yang dihapus bisa dipulihkan dari sini.' : 'Belum ada permission yang cocok dengan filter.'}
