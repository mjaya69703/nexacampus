...
{
    public CampusAsset $asset;
    public array $form = [];

    public function mount(int $id): void
    {
        $this->asset = CampusAsset::findOrFail($id);
        $this->form = [
            'asset_code' => $this->asset->asset_code,
            'name' => $this->asset->name,
            'category' => $this->asset->category,
            'room_id' => $this->asset->room_id,
            ...
            'is_borrowable' => $this->asset->is_borrowable,
    ...
    public function save(): void
    {
        $validated = $this->validate([
            'form.asset_code' => 'required|string|max:50|unique:campus_assets,asset_code,' . $this->asset->id,
...
<div class="w-full" style="width: 100% !important">
    ...
    <x-admin.campus.header
        title="Edit Aset Kampus"
        description="Perbarui data spesifikasi, lokasi penempatan, atau status kondisi fisik aset."
        icon="boxes-packing"
    >
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
                <div class="card-header bg-white border-bottom p-4">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h4 class="card-title fw-bold mb-1 text-dark">Edit Aset: {{ $asset->asset_code }}</h4>
                            <div class="text-muted small">Perbarui data aset dan simpan perubahan.</div>
                        </div>
                    </div>
                </div>
                    ...
                    <form wire:submit.prevent="save">
                        <div class="row g-3">
                            ...
                            <div class="col-lg-4">
                                ...
                                <input type="text" class="form-control rounded-3 @error('form.brand') is-invalid @enderror" wire:model="form.brand">
            ...
            <div class="card border-0 shadow-sm rounded-4 mb-4 overflow-hidden">
                <div class="card-body p-4 bg-light bg-opacity-50">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        ...
                        <div>
                            <h5 class="fw-bold mb-1">Status Aset</h5>
                            <div class="text-muted small">Informasi kondisi barang.</div>
                        </div>
                    </div>

                    <div class="border rounded-3 p-3 bg-white mb-3">
                        <div class="text-muted small mb-1">Kondisi Fisik</div>
                        <div class="fw-bold text-dark">{{ ucfirst(str_replace('_', ' ', $asset->condition)) }}</div>
                    </div>
                </div>
            </div>
        </div>
