...
new class extends Component
{
    ...
    {
        $this->validate([
            'permissionForm.name' => 'required|string|max:255|unique:permissions,name,' . $this->permissionId . ',id,guard_name,' . $this->permissionForm['guard_name'],
        ...
        session()->flash('success', 'Permission "' . $this->permissionForm['name'] . '" berhasil diperbarui.');
        $this->redirectRoute('admin.access.permissions.index');
    }
...
<div class="w-full" style="width: 100% !important">
    ...
    <x-admin.access.header
        title="Edit Permission: {{ $permissionForm['name'] ?? '' }}"
        description="Perbarui nama permission, guard, dan mapping role tanpa mengubah alur akses yang sudah berjalan."
        icon="shield-halved"
    >
                ...
                <div class="card-header bg-white border-bottom p-3 p-md-4">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            ...
                            <div class="text-muted small">Sesuaikan metadata permission dan role yang berhak menggunakannya.</div>
                        </div>
                    </div>
                </div>

                <div class="card-body p-4">
                    <form wire:submit.prevent="updatePermission" class="row g-3">
                        <div class="col-lg-6 col-md-6 col-sm-12">
                            <label for="name" class="form-label fw-semibold text-dark">Nama Permission <span class="text-danger">*</span></label>
                            <input type="text" id="name" class="form-control rounded-3" wire:model.defer="permissionForm.name">
                            @error('permissionForm.name') <small class="text-danger d-block mt-1">{{ $message }}</small> @enderror
                            <small class="text-muted d-block mt-1">Contoh: user.viewAny, user.create, user.update.</small>
                        </div>
                                ...
                                <div wire:ignore>
                                    <select id="roles-select" class="form-control" multiple>
                                        @foreach($availableRoles as $roleId => $roleName)
                                            <option value="{{ $roleId }}" @selected(in_array($roleId, $selectedRoles, true))>{{ ucfirst($roleName) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                ...
                <div class="card-body p-4 bg-light bg-opacity-50">
                    <div class="d-flex align-items-center gap-3 mb-3">
                        ...
                        <div>
                            ...
                            <div class="text-muted small">Jaga nama permission tetap konsisten agar permission table dan role mapping tetap mudah dirawat.</div>
                        </div>
                    </div>

                    <ul class="list-unstyled mb-0 text-muted small d-grid gap-2">
                        <li class="d-flex gap-2"><i class="fa fa-check text-success mt-1"></i><span>Gunakan nama yang mengikuti pola resource.action.</span></li>
                        <li class="d-flex gap-2"><i class="fa fa-check text-success mt-1"></i><span>Perubahan role mapping langsung berpengaruh ke hak akses user.</span></li>
                        <li class="d-flex gap-2"><i class="fa fa-check text-success mt-1"></i><span>Guard harus tetap sesuai konteks autentikasi modul.</span></li>
                    </ul>
                </div>
            </div>
        </div>
