...
new class extends Component
{
    ...
    public function mount(int $id): void
    {
        ...
        $this->categoryId = $this->news->category_id ? (string) $this->news->category_id : null;
        $this->excerpt = $this->news->excerpt ?? '';
        $this->content = $this->news->content;
        $this->isPublished = (bool) $this->news->is_published;
        $this->publishedAt = $this->news->published_at?->format('Y-m-d\TH:i');
        $this->existingImage = $this->news->featured_image;
    }
    ...
    public function save(): void
    {
        $this->validate([
            ...
            'slug' => 'required|string|max:255|unique:news,slug,' . $this->news->id,
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden mb-4">
                <div class="card-header bg-white border-bottom p-4">
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div>
                            <h4 class="card-title fw-bold mb-1 text-dark">Form Edit Berita</h4>
                            <div class="text-muted small">Perbarui data berita secara lengkap.</div>
                        </div>
                    </div>
                </div>
                <div class="card-body p-4">
                    ...
                    <div class="mb-0">
                        ...
                        <livewire:jodit-text-editor wire:model.live="content" identifier="news-edit-content" :height="400" />
                        @error('content')
                            <div class="text-danger small mt-1">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="card-body p-4">
                    <div class="mb-0">
                        ...
                        @elseif($existingImage)
                            <div class="mt-3 position-relative">
                                <img src="{{ Storage::url($existingImage) }}" class="img-fluid rounded-3 border" alt="Current featured image">
                                <span class="badge bg-secondary mt-2 d-inline-block">Gambar saat ini</span>
                            </div>
                        @endif
                    </div>
                </div>
