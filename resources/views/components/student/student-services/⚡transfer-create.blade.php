...
new class extends Component
{
    ...
    public function mount(): void
    {
        ...
        $this->form['recommended_semester'] = $studentProfile?->current_semester;
    }
    ...
    public function save(StudentTransferRequestService $service): void
    {
        ...
        try {
            $request = $service->create($studentProfile, $validated, $this->attachment);
            session()->flash('success', 'Pengajuan transfer berhasil dikirim.');
...
<div>
            ...
            <div class="d-flex align-items-start justify-content-between gap-3 flex-wrap">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div>
                        ...
                        <h1 class="h2 mb-2" style="font-weight: 800;">Ajukan Pindah Program</h1>
                        <div style="opacity: 0.9;">Pengajuan ini akan direview manual oleh admin akademik sebelum diterapkan.</div>
                    </div>
                </div>
    ...
    <div class="card service-card">
        <div class="card-header d-flex justify-content-between align-items-center" style="background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); border-bottom: 2px solid #e2e8f0;">
            <div>
                <h3 class="card-title mb-0">Form Pengajuan Transfer</h3>
                <small class="text-muted d-block mt-1">Isi tujuan pindah, alasan, dan lampiran pendukung jika ada.</small>
            </div>
        </div>
        <div class="card-body p-4">
            <form wire:submit.prevent="save">
                @include('components.student.student-services.transfer-form')
            </form>
        </div>
    </div>
</div>
